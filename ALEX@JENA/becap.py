...
def skeletisation_1(arr, x, y, f):
    ...
    string = [str(one_null(arr, m, k)) for (m, k) in circ]
    string1 = ''.join(string)
    print(string , string1 , string1.count('01'))
...
def skeletisation_2(arr, x, y):
    ...
    return (array[0] + sum(array[4:8]) == 0 and (array[1] + array[3]) >= 255 and array[2] == 255) or \
        ...
        (sum(array[4:8]) + array[0] == 5 * 255 and sum(array[0:8]) == 5 * 255)
...
for i in range(len(paths)):
    ...
    for m, k in paths[i]:
        if if_first:
            ...
            coef = coef/2
            if (count + 1 == len(paths[i])):
                usm1.append(temp_x)
                usm2.append(temp_y)
            count+=1

fin_vec = list(zip(usm1, usm2))
...
for m, k in fin_vec:
    if count % 2 == 1:
        ...
        lp = True
    if lp:
        bend_cos.append(((i1 * i2) + (j1 * j2)) / (d * r))
    count += 1
...
for i in range(len(paths)):
    ...
    for m,k in paths[i]:
        ...
        if len(paths[i]) == count+1 :
            ...
            end_y2 = k
            if not if_composite:
                temp_coef = len(paths[i]) / (np.sqrt((end_x2 - start_x1)**2 + (end_y2 - start_y1)**2))
                print(temp_coef)
